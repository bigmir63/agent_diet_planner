...
PROJECT_ID = os.environ.get("GCP_PROJECT_ID", "testai-ca58f")
...
def generate_diet_plan(params):
   try:
       ...
       prompt = f"""
       아래 정보를 바탕으로 개인화된 7일 Diet 계획을 한글로 작성하세요:
...
def main():
    ...
    with st.form("user_profile", clear_on_submit=False):
        st.markdown("<h3 style='color: black;'>아래 사항을 입력하세요</h3>", unsafe_allow_html=True)
        ...
        with col2:
           ...
           gender = st.selectbox("성", ["Man", "Woman"])


        # col1, col2 = st.columns(2)

        # with col1:
        #     st.markdown("<h6 style='color:red;'>몸무게 (kg)</h6>", unsafe_allow_html=True)
        #     weight = st.number_input(" ", min_value=30.0, max_value=200.0, value=70.0)

        #     st.markdown("<h6 style='color:red;'>신장 (cm)</h6>", unsafe_allow_html=True)
        #     height = st.number_input(" ", min_value=100, max_value=250, value=170)

        # with col2:
        #     st.markdown("<h6 style='color:red;'>나이</h6>", unsafe_allow_html=True)
        #     age = st.number_input(" ", min_value=18, max_value=100, value=30)

        #     st.markdown("<h6 style='color:red;'>성별</h6>", unsafe_allow_html=True)
        #     gender = st.selectbox(" ", ["Man", "Woman"])

        submitted = st.form_submit_button("Diet 계획 생성")
    ...
    if submitted and st.session_state.user_data:
         with st.spinner("🧠 AI가 당신을 위한 다이어트 계획을 작성하고 있습니다..."):
    ...
    #    client_email = "agentic-ai@testai-ca58f.iam.gserviceaccount.com"
       
        # Show history
        st.subheader("당신의 Plan History")
        try:
            query = f"""
